...
# -------------------------------------------------------------------------------------------
    ...
    def div_grille(self, longG, largG, listRed, listTrou):
        ...
        with open('Redondants_g.txt', 'w') as f:
            f.write('[')
            for item in listR:
                f.write(f'{item},\n')
            f.write(']')

        with open('Redondants_gg.txt', 'w') as f:
            f.write('[')
            for item in listRR:
                f.write(f'{item},\n')
            f.write(']')

        with open('Trous_g.txt', 'w') as f:
            f.write('[')
            for item in listTT:
                 f.write(f'{item},\n')
            f.write(']')
        ...
        for g in listTrou :
            for u in g[2] :
                if self.CanCommunicate(monX, monY,u[0], u[1]) and len(self.chargeR) > 0 and (u[0], u[1]) not in Trou_regle:            
                    ...
                    t.goto(X,Y)
                    debut = time.time()
                    #le nombre de Pixel ajouté lorsque j'nstalle ce capteur a la position X,Y
...
#--------------------------Marche-------------------------------------------
    def marche(self, listCapteur, listRed, listTrou, initial_angle, temps_perdu):
        ...
        with open('liste_red.txt', 'w') as f:
            f.write('[')
            for item in listRed:
                f.write(str(item)+',\n')
            f.write(']')
        with open('liste_trou_regle.txt', 'w') as f:
            f.write('[')
            for item in Trou_regle:
                f.write(str(item)+',\n')
            f.write(']')
    ...
    def LRV(self, listCapteur, listRed, listTrou, initial_angle, temps_perdu):
        ...
        with open('liste_trou_regle.txt', 'w') as f:
            ...
            for item in Trou_regle:
                f.write(f'{item},\n')
            f.write(']')
        with open('liste_pixel.txt', 'w') as f:
            f.write('[')
